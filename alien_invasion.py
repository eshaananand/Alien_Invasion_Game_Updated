...
class AlienInvasion:
    ...
    def leaderboard(self):
        with open ('score.txt','r') as f1:
            score1=int(f1.readline())
            
        with open('leaderboard.txt', 'r') as f:
            top_scores = []
            for line in f:
                number = int(line.strip())
                top_scores.append(number)
                top_scores = sorted(top_scores, reverse=True)[:5]

        present_high_score = score1

        if present_high_score > top_scores[-1]:
            top_scores.append(present_high_score)
            top_scores = sorted(top_scores, reverse=True)[:5]
        ...
        for i, score in enumerate(top_scores):
            ...
            if rank == 1:
                ...
                suffix = "rd"
            print(f"{rank}{suffix} highest: {score}")
